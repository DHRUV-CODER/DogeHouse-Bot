...
class Client(dogehouse.DogeClient):
    ...
    async def on_ready(self):
        ...
        # await self.create_room("Dhruv's Bot Room -> 24/7")
        await self.join_room("61ed2b52-8b75-4132-91ae-ceaa85b015ca")
        print("Joined")
    ...
    async def help(self,ctx):
        ...
        await self.send(message="Cmds -> !hanimal , !hcryptog , !hjoke",whisper=whisper_that_g)

    @dogehouse.command
    async def hanimal(self,ctx):
        whisper_that_g = [ctx.author.id]
        await self.send(message="!catfact, !dogfact, !pandafact, !foxfact, !birdfact, !koalafact",whisper=whisper_that_g)

    @dogehouse.command
    async def hcryptog(self,ctx):
        whisper_that_g = [ctx.author.id]
        await self.send(message="!encode <msg>, !decode <msg>",whisper=whisper_that_g)
    ...
    async def encode(self,ctx,*,encode_msg="no_msg"):
        resp = r.get(f"https://some-random-api.ml/base64?encode={encode_msg}").json()
        await self.send(message=f"->  {resp['base64']}",whisper=[ctx.author.id])     

    @dogehouse.command
    async def decode(self,ctx,*,decode_msg="bm8gbXNn"):
        resp = r.get(f"https://some-random-api.ml/base64?decode={decode_msg}").json()
    ...
    async def reg(self,ctx,*,act):
        ...
        with open("currentAct.json", "w+") as f:
            ...
            await self.send(message=f"Done!!, Actvity Changed To -> {act}",whisper=[ctx.author.id])
    ...
    async def on_user_join(self,user):
        ...
        await self.send(message=f"welcome `{user.username}` !! , Pls Type `!help` For More Info & Btw If Udk What We Doin Type -> `!what_we_doin`",whisper=userNameToWhisper)
